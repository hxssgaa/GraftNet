...
class RelOrderReasonerDataLoader():
    def __init__(self, data_file, facts, num_hop, max_relation, word2id, relation2id, max_query_word, use_inverse_relation, div, pred_data=None, teacher_force=False):
        ...
        self.num_paths = max_relation ** num_hop
        ...
        self.local_kb_rel_path_rels = np.zeros((self.num_data, self.num_paths, self.num_hop))
        ...
        self.answer_dists = np.zeros((self.num_data, self.num_paths), dtype=float)
    ...
    def _prepare_data(self):
        ...
        for idx_q, sample in tqdm(enumerate(self.data)):
            ...
            rel_ids = set()
            rel_id_path = []
            if 'rel_path_pred' in sample:
                keyword = 'rel_path_pred'
            else:
                keyword = 'rel_path'
            for rel in sample[keyword]:
                rel_ids.add(self.relation2id[rel])
            # This is for the evaluation purpose
            for rel in sample['rel_path']:
                rel_id_path.append(self.relation2id[rel])
            rel_id_path = tuple(rel_id_path)

            # get a list of relation candidate paths
            rel_permutations = [p for p in itertools.product(list(rel_ids), repeat=self.num_hop)]
            for i in range(len(rel_permutations)):
                rel_permutation = rel_permutations[i]
                for hop in range(self.num_hop):
                    self.local_kb_rel_path_rels[idx_q][i][hop] = rel_permutation[hop]
                if rel_permutation == rel_id_path:
